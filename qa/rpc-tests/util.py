...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "french.conf"), 'w') as f:
        f.write("regtest=1\n");
        f.write("rpcuser=rt\n");
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    frenchd and french-cli must be in search path.
    """
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in french.conf
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None):
    """
    Start multiple frenchds, return RPC connections to them
    """
