...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FrenchMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FrenchMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FrenchMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("french-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running FrenchMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
