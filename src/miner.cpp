...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FrancMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FrancMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FrancMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("franc-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running FrancMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
