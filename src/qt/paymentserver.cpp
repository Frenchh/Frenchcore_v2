...
const char* BIP71_MIMETYPE_PAYMENT = "application/french-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/french-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/french-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        ...
        // If the french: URI contains a payment request, we are not able to detect the
                                                                       ...
                                                                       optionsModel(0)
{
    ...
    // on Mac: sent when you click french: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start french: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling french: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject* object, QEvent* event)
{
    // clicking on french: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in french: URIs
