...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::FRC, nValue);
...
{
    std::string genesisblockhash = "00000384e57c8b5e6a33c32f2649aa92dbac025609079f8e55dcc72890df1c7c";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (french.conf).");
            QMessageBox::warning(this, "French Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
